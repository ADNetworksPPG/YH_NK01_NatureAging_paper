...
pdf(paste0(outdir,"PCA_logOdds_by_genotype_all_subclusters.pdf"))
...
logOdds_histo_astro <- logOdds_histo_astro[, c(1:13,15,16,14)]
...
p <- fviz_pca_ind(res.pca.norm.histo,
             geom.ind = c("point"),
                ...
                guides(x.sec = "axis", y.sec = "axis")+
                scale_fill_manual(values = c("gold", "pink2","forestgreen",getPalette(nb.cols)))+
                #geom_text(aes(label=ifelse(data.frame(res.pca.norm.histo$x)$PC1>2 & data.frame(res.pca.norm.histo$x)$PC2>0,as.character(pca_histo_log$cluster),'')),position = position_dodge(0.9))
                 annotate("text", label = "MG cluster 8", x = -1.3 ,y = 1.4, size = 4) +
                 annotate("text", label = "MG cluster 6", x = 4.9 ,y = 1.5, size = 4) +
                 annotate("text", label = "AS cluster 5", x = 1.7 ,y = 1.55, size = 4) +
                 annotate("text", label = "Cluster 15", x = 3.6 ,y = 1.82, size = 4) +
                 annotate("text", label = "AS cluster 3", x = 1.6 ,y = 0.94, size = 4) +
                 annotate("text", label = "Cluster 7", x = 2.8 ,y = 1.32, size = 4) +
                 annotate("text", label = "Cluster 18", x = 0.87 ,y = 1.2, size = 4)
  ggsave(file = file.path(outdir, paste0("PCA_logodds_histopathology_cl7_15_18_all_subclusters_refined.pdf")),
         plot = p,
         width = 10,
         height = 10)
