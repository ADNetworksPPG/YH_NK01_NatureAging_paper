...
sourcedir<-sourcedir<-"~/Dropbox (Gladstone)/YH_NK01-Results/11_seurat_analysis_filter_percent_mt_noGFAPCre/06_analysis_of_cluster_of_interest/log_odds_calculation_sub_cluster_astrocytes_cluster_12/"
...
colnames(ClusterRes)[c(1:4,9:16)] <- c("logOddsRatio_for_unit_Hippocampus_Vol_mm3",
...
Cluster13<-data.frame(logOddsRatio_for_unit_Hippocampus_Vol_mm3=NA, logOddsRatio_for_unit_prop_AT8_Coverage_Area=0.1769158,logOddsRatio_for_unit_prop_MBP_Coverage_Area=-0.1776881, logOddsRatio_for_unit_prop_OPC_Coverage_Area=-0.3308154,
StdErr_for_unit_Hippocampus_Vol_mm3=NA, StdErr_for_unit_prop_AT8_Coverage_Area=0.0538625, StdErr_for_unit_prop_MBP_Coverage_Area=0.4293817, StdErr_for_unit_prop_OPC_Coverage_Area=0.39269089 ,X5=NA, X6=3.2845826,X7= -0.4138227,X8= -0.8424298, pvalue_for_unit_Hippocampus_Vol_mm3=NA,pvalue_for_unit_prop_AT8_Coverage_Area=0.001021344, pvalue_for_unit_prop_MBP_Coverage_Area=0.67900365, pvalue_for_unit_prop_OPC_Coverage_Area=0.39954619 )                                      
ClusterRes <- rbind(ClusterRes,Cluster13)
row.names(ClusterRes)[15] <-  "Cluster13"
...
##output the results
ClusterRes <- ClusterRes[,!(colnames(ClusterRes) %in% c("X5","X6","X7","X8"))]
print(ClusterRes)
ClusterRes<- ClusterRes[c(1:12,15,14,13),]
write.csv(t(ClusterRes), file = paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_subcluster_astrocytes_subset_pheno.csv"))
...
histo_all<-read.csv(paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_subcluster_astrocytes_subset_pheno.csv"))
...
#subset with cluster of interest  1, 5
histo_no_norm_cluster_of_interest <-  histo_no_norm %>% select(Cluster1,Cluster5)
histo_rowMeans_norm_cluster_of_interest <-  histo_rowMeans_norm %>% select(Cluster1,Cluster5)
...
pdf(paste0(plotdir,"Fig.4e_astrocytes_clusters_pathology_cl1_5.pdf"), height=6, width=12)
...
pdf(paste0(plotdir,"Fig.4e_suppl_astrocyte_clusters_pathology_15_clusters.pdf"), height=4, width=24)
print(pheatmap(no_norm_histo_kmeans, cluster_rows=F, show_rownames=T, show_colnames=T, cluster_cols=F, treeheight_row=T,  gaps_row = F, annot_cols=df,annot_row=cl, color = simpleredbluecols, breaks = myBreaks, display_numbers = TRUE, number_format="%.3f",
         number_color = "black", 
         fontsize_number = 10))
dev.off()
