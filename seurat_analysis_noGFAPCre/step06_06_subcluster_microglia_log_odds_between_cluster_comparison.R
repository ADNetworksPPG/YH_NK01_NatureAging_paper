...
outdir <- paste0(basedir, 
                 "log_odds_calculation_sub_cluster_",
                 "microglia_clusters_14_25_29/")
...
write.csv(smp_cluster_counts, 
          file = "counts_per_sample_per_cluster_subcluster_microglia.csv",
          row.names = FALSE)

counts <- read.csv("counts_per_sample_per_cluster_subcluster_microglia.csv", 
                   header = T)
...
estimateCellStateChange <- function(k, counts, pheno) {
  ...
  pdf(paste0("proportion_of_cells_per_genotype_cluster_subcluster_microglia_",
             k,
             ".pdf"))
  ...
  glmerFit <- glmer(InCluster ~ (1|ID) + animal_model, data=FitData, 
                    family = "binomial", 
...
write.csv(ClusterRes, 
          file = "log_odds_ratio_per_genotype_per_cluster_subcluster_microglia.csv")
...
if(!(nrow(label.df2)==0)){label.df2$logOddsRatio <- 4.0}
pdf("log_odds_barplot_subcluster_microglia_with_padj.pdf",
    height = 20,
    width = 35
)
...
pdf("log_odds_barplot_subcluster_microglia_with_pvalue_and_padj.pdf",
    height = 20,
    width = 35
)
ggplot(ClusterRes_plot, 
