...
pdf(paste0(plotdir,"proportion_of_cells_per_sample_astrocytes_subcluster_",k,"_per_pheno_",colnames(cluster_counts[ph]),".pdf"))
...
print(ClusterRes)
write.csv(t(ClusterRes), file = paste0(outdir,"log_odds_ratio_opt_bobyqa_per_unit_per_histopathology_per_subcluster_astrocytes.csv"))
...
ClusterRes <- ClusterRes[,!(colnames(ClusterRes) %in% c("X17","X18","X19","X20","X21","X22","X23","X24"))]
print(ClusterRes)
write.csv(t(ClusterRes), file = paste0(outdir,"log_odds_ratio_opt_bobyqa_per_unit_per_histopathology_per_subcluster_microglia.csv"))
