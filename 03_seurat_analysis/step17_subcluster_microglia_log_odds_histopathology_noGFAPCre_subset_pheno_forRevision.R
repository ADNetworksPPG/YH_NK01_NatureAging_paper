...
sourcedir<-sourcedir<-"~/Dropbox (Gladstone)/YH_NK01-Results/11_seurat_analysis_filter_percent_mt_noGFAPCre/06_analysis_of_cluster_of_interest/log_odds_calculation_sub_cluster_microglia_clusters_14_25_29/"
...
print(ClusterRes)
ClusterRes<- ClusterRes[c(1:5,15,6:14),]
write.csv(t(ClusterRes), file = paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_subcluster_microglia_subset_pheno.csv"))
...
histo_all<-read.csv(paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_subcluster_microglia_subset_pheno.csv"))
...
histo_rowMeans_norm_cluster_of_interest <-  histo_rowMeans_norm %>% select(luster4,Cluster6, Cluster8)
...
pdf(paste0(plotdir,"Fig.8h_microglia_clusters_pathology_cl4_6_8.pdf"), height=6, width=12)
...
pdf(paste0(plotdir,"Fig.8h_suppl_microglia_clusters_pathology_15_clusters.pdf"), height=4, width=24)
print(pheatmap(no_norm_histo_kmeans, cluster_rows=F, show_rownames=T, show_colnames=T, cluster_cols=F, treeheight_row=T,  gaps_row = F, annot_cols=df,annot_row=cl, color = simpleredbluecols, breaks = myBreaks, display_numbers = TRUE, number_format="%.3f",
         number_color = "black", 
         fontsize_number = 10))
dev.off()
